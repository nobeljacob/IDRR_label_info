...
def add_dictionary_to_excel(dictionary, filename, sheetname):
    ...
    if sheetname in workbook.sheetnames:
        ...
        # Write the dictionary keys as column headers
    ...
    def load_csv(self, csv_file_path, processed_dir, sections=None, types=None):
        ...
        for idx, row in tqdm(df.iterrows()):
            ...
            x = {"id": "%d_%s_wsj_%02d%02d" % (idx, row[0], row[1], row[2]),
                 ...
                 "arg1": row[8],
                 "arg2": row[9]}
            to_write_dict.append(x)


        # add_dictionary_to_excel(x, 'existing_file.xlsx', 'Sheet1')
        # convert into dataframe
        df = pd.DataFrame(data=to_write_dict)
...
class Sampler(data.Sampler):
    ...
    def make_array(self):
        ...
        array = array.view(
            list(zip(list(self.cache.keys()) + ["rand"], [np.float32] * (len(self.cache) + 1)))).flatten()
